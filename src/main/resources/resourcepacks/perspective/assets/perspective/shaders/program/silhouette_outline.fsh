...
void main() {
    ...
    float outlineFactor = clamp(pow(max(2.0 * 0.025 * 1000.0 / (1000.0 + 0.025 - (max(depth00, max(depth01, max(depth11, depth10))) * 2.0 - 1.0) * (1000.0 - 0.025)) - worldDepth, 0.0), 2.0), 0.0, 1.0) * exp(-worldDepth * 0.025);
