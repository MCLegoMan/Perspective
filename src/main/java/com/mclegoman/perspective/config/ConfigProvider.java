...
public class ConfigProvider implements SimpleConfig.DefaultConfig {
	...
	public void add(Pair<String, ?> keyValuePair) {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to add {} config value: {}", keyValuePair, error));
		}
	}
	@Override
	...
	public void setConfig(String KEY_NAME, Object KEY_VALUE) {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to set {} config value: {}", KEY_NAME, error));
		}
	}
	public void saveConfig(String ID) {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to save {} config: {}", ID, error));
		}
	}
}