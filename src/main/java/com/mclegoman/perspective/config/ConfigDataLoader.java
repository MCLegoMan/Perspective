	...
	public void apply(Map<Identifier, JsonElement> prepared, ResourceManager manager, Profiler profiler) {
		try {
			for (Resource resource : manager.getAllResources(new Identifier(Data.version.getID(), ID + ".json"))) {
				ZOOM_LEVEL = net.minecraft.util.JsonHelper.getInt(net.minecraft.util.JsonHelper.deserialize(resource.getReader()), "zoom_level", 40);
				ZOOM_INCREMENT_SIZE = net.minecraft.util.JsonHelper.getInt(net.minecraft.util.JsonHelper.deserialize(resource.getReader()), "zoom_increment_size", 2);
				...
				SUPER_SECRET_SETTINGS_SHADER = net.minecraft.util.JsonHelper.getString(net.minecraft.util.JsonHelper.deserialize(resource.getReader()), "super_secret_settings_shader", "minecraft:none");
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to load default {} config values: {}", ID, error));
		}
	}
	@Override
