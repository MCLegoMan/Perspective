...
public class ConfigHelper {
	...
	public static void init() {
		try {
			...
			Data.version.sendToLog(LogType.ERROR, Translation.getString("Failed to initialize config: {}", error));
		}
	}
	protected static void loadConfig() {
		try {
			...
			Data.version.sendToLog(LogType.ERROR, Translation.getString("Failed to load configs: {}", error));
		}
	}
	public static void tick() {
		try {
			...
			Data.version.sendToLog(LogType.ERROR, Translation.getString("Failed to tick configs: {}", error));
		}
	}
	private static void showToasts() {
		if (Data.version.isDevelopmentBuild() && !SEEN_DEVELOPMENT_WARNING) {
			Data.version.sendToLog(LogType.WARN, Translation.getString("Development Build. Please help us improve by submitting bug reports if you encounter any issues."));
		...
		if (SHOW_DOWNGRADE_WARNING && !SEEN_DOWNGRADE_WARNING) {
			Data.version.sendToLog(LogType.WARN, Translation.getString("Downgrading is not supported. You may experience configuration related issues."));
		...
		if (SHOW_LICENSE_UPDATE_NOTICE && !SEEN_LICENSE_UPDATE_NOTICE) {
			Data.version.sendToLog(LogType.WARN, Translation.getString("License Update. {} is now licensed under LGPL-3.0-or-later.", Data.version.getName()));
	...
	public static void updateConfig() {
		try {
			...
			Data.version.sendToLog(LogType.WARN, Translation.getString("Failed to update config: {}", error));
		}
	}
	public static void saveConfig(boolean onTick) {
		try {
			...
			Data.version.sendToLog(LogType.WARN, Translation.getString("Failed to save config: {}", error));
		}
	}
	public static void resetConfig() {
		try {
			...
			setConfig("force_pride_type_index", MathHelper.clamp(ConfigDataLoader.FORCE_PRIDE_TYPE_INDEX, 0, PerspectiveLogo.prideTypes.size()));
			...
			Data.version.sendToLog(LogType.WARN, Translation.getString("Failed to reset config: {}", error));
		}
	}
		...
		try {
			switch (ID) {
				...
				case "force_pride_type_index" ->
						Config.FORCE_PRIDE_TYPE_INDEX = MathHelper.clamp((int)VALUE, 0, PerspectiveLogo.prideTypes.size());
	...
	public static void setExperimentalConfig(String ID, Object VALUE) {
		try {
			...
			Data.version.sendToLog(LogType.WARN, Translation.getString("Failed to set {} experimental config value: {}", ID, error));
		}
	}
	public static void setTutorialConfig(String ID, Object VALUE) {
		try {
			...
			Data.version.sendToLog(LogType.WARN, Translation.getString("Failed to set {} tutorial config value: {}", ID, error));
		}
	}
	public static void setWarningConfig(String ID, Object VALUE) {
		try {
			...
			Data.version.sendToLog(LogType.WARN, Translation.getString("Failed to set {} warning config value: {}", ID, error));
		}
	}
	public static Object getConfig(String ID) {
		switch (ID) {
			...
			case "force_pride_type_index" -> {
				return MathHelper.clamp(Config.FORCE_PRIDE_TYPE_INDEX, 0, PerspectiveLogo.prideTypes.size());
			}
	...
	public static Object getExperimentalConfig(String ID) {
		switch (ID) {
			...
			default -> {
				Data.version.sendToLog(LogType.WARN, Translation.getString("Failed to get {} experimental value: Invalid Key", ID));
				return new Object();
			}
		}
	}
