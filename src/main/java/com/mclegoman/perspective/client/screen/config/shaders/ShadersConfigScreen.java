...
public class ShadersConfigScreen extends Screen {
	...
	public void init() {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to initialize config>shaders screen: {}", error));
		}
	}
	public void tick() {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to tick config>shaders screen: {}", error));
		}
	}
