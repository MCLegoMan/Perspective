...
public class ZoomConfigScreen extends Screen {
	...
	public void init() {
		try {
			...
			GRID_ADDER.add(ConfigScreenHelper.createTitle(ClientData.minecraft, new ZoomConfigScreen(PARENT_SCREEN, true), true, "zoom", false));
	...
	public void tick() {
		try {
			if (this.REFRESH) {
				ClientData.minecraft.setScreen(new ZoomConfigScreen(PARENT_SCREEN, false));
			}
	...
	private GridWidget createZoom() {
		...
		GRID_ADDER.add(new SliderWidget(GRID_ADDER.getGridWidget().getX(), GRID_ADDER.getGridWidget().getY(), 150, 20, Translation.getConfigTranslation(Data.version.getID(), "zoom.level", new Object[]{Text.literal((int) ConfigHelper.getConfig("zoom_level") + "%")}, false), ZOOM_LEVEL) {
			@Override
		...
		GRID_ADDER.add(new SliderWidget(GRID_ADDER.getGridWidget().getX(), GRID_ADDER.getGridWidget().getY(), 150, 20, Translation.getConfigTranslation(Data.version.getID(), "zoom.increment_size", new Object[]{Text.literal(String.valueOf((int) ConfigHelper.getConfig("zoom_increment_size")))}, false), ZOOM_INCREMENT_SIZE) {
			@Override
			protected void updateMessage() {
		...
		GRID_ADDER.add(ButtonWidget.builder(Translation.getConfigTranslation(Data.version.getID(), "zoom.transition", new Object[]{Translation.getZoomTransitionTranslation(Data.version.getID(), (String) ConfigHelper.getConfig("zoom_transition"))}), (button) -> {
			ConfigHelper.setConfig("zoom_transition", Zoom.nextTransition());
			this.REFRESH = true;
		}).build(), 1);
		GRID_ADDER.add(ButtonWidget.builder(Translation.getConfigTranslation(Data.version.getID(), "zoom.camera_mode", new Object[]{Translation.getZoomScaleModeTranslation(Data.version.getID(), (String) ConfigHelper.getConfig("zoom_camera_mode"))}), (button) -> {
			ConfigHelper.setConfig("zoom_camera_mode", Zoom.nextCameraMode());
			this.REFRESH = true;
		}).build(), 1);
		GRID_ADDER.add(ButtonWidget.builder(Translation.getConfigTranslation(Data.version.getID(), "zoom.hide_hud", new Object[]{Translation.getVariableTranslation(Data.version.getID(), (boolean) ConfigHelper.getConfig("zoom_hide_hud"), Translation.Type.ONFF)}), (button) -> {
			ConfigHelper.setConfig("zoom_hide_hud", !(boolean) ConfigHelper.getConfig("zoom_hide_hud"));
			this.REFRESH = true;
		}).build(), 1);
		GRID_ADDER.add(ButtonWidget.builder(Translation.getConfigTranslation(Data.version.getID(), "zoom.show_percentage", new Object[]{Translation.getVariableTranslation(Data.version.getID(), (boolean) ConfigHelper.getConfig("zoom_show_percentage"), Translation.Type.ONFF)}), (button) -> {
