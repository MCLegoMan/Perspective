...
public class ShaderSelectionConfigScreen extends Screen {
	...
	protected void init() {
		...
		if (!(boolean) ConfigHelper.getExperimentalConfig("super_secret_settings_notice")) {
			ClientData.CLIENT.getToastManager().add(new Toast(Translation.getTranslation(Data.version.getID(), "toasts.experimental.title", new Object[]{Translation.getTranslation(Data.version.getID(), "name")}), Translation.getTranslation(Data.version.getID(), "toasts.experimental.description", new Object[]{Translation.getTranslation(Data.version.getID(), "experimental.super_secret_settings_list")}), 280, Toast.Type.WARNING));
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		context.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
		context.drawCenteredTextWithShadow(ClientData.CLIENT.textRenderer, Translation.getConfigTranslation(Data.version.getID(), "shaders.shader.select", new Formatting[]{randomColor}), ClientData.CLIENT.getWindow().getScaledWidth() / 2, 12, 0xFFFFFF);
