...
public class HideConfigScreen extends Screen {
	...
	public void init() {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to initialize>hide config screen: {}", error));
		}
	}
	public void tick() {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to tick>hide config screen: {}", error));
		}
	}
