...
public class ExperimentalConfigScreen extends Screen {
	...
	public void init() {
		try {
			...
			Data.version.sendToLog(LogType.ERROR, Translation.getString("Failed to initialize>experimental config screen: {}", error));
		}
	}
	public void tick() {
		try {
			...
			Data.version.sendToLog(LogType.ERROR, Translation.getString("Failed to tick>experimental config screen: {}", error));
		}
	}
