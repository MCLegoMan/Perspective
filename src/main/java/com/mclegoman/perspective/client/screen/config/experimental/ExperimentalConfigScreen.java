...
public class ExperimentalConfigScreen extends Screen {
	...
	public void init() {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to initialize>experimental config screen: {}", error));
		}
	}
	public void tick() {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to tick>experimental config screen: {}", error));
		}
	}
	...
	private GridWidget createExperiments() {
		...
		GRID.getMainPositioner().alignHorizontalCenter().margin(2);
		GridWidget.Adder GRID_ADDER = GRID.createAdder(2);
		GRID_ADDER.add(new MultilineTextWidget(Translation.getConfigTranslation(Data.version.getID(), "experimental.warning", new Formatting[]{Formatting.RED, Formatting.BOLD}), ClientData.minecraft.textRenderer).setCentered(true), 2);
