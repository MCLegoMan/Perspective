...
public class UpdateCheckerScreen extends Screen {
	...
	public void init() {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to initialize config>update screen: {}", error));
		}
	}
	public void tick() {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to tick config>update screen: {}", error));
		}
	}
