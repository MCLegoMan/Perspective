...
public class HoldPerspectiveConfigScreen extends Screen {
	...
	public void init() {
		try {
			...
			Data.version.sendToLog(LogType.ERROR, Translation.getString("Failed to initialize>hold_perspective config screen: {}", error));
		}
	}
	public void tick() {
		try {
			...
			Data.version.sendToLog(LogType.ERROR, Translation.getString("Failed to tick>hold_perspective config screen: {}", error));
		}
	}
