...
public class HideArmorDataLoader extends JsonDataLoader implements IdentifiableResourceReloadListener {
	...
	public void apply(Map<Identifier, JsonElement> prepared, ResourceManager manager, Profiler profiler) {
		try {
			...
			Data.version.sendToLog(LogType.ERROR, Translation.getString("Failed to apply hide armor to registry: {}", error));
		}
	}
	@Override
	...
	private void layout$perspective(ResourceManager manager) {
		...
		for (Resource resource : HIDE_LISTS) {
			try {
				...
				Data.version.sendToLog(LogType.ERROR, Translation.getString("Failed to load hide armor registry: {}", error));
			}
		}
	}
}