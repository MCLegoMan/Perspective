...
public class HidePlayerDataLoader extends JsonDataLoader implements IdentifiableResourceReloadListener {
	...
	private void add(String value) {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to add hide player to registry: {}", error));
		}
	}
	private void reset() {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to reset hide player registry: {}", error));
		}
	}
	@Override
	public void apply(Map<Identifier, JsonElement> prepared, ResourceManager manager, Profiler profiler) {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to apply hide player registry: {}", error));
		}
	}
	@Override
	...
	private void layout$perspective(ResourceManager manager) {
		...
		for (Resource resource : HIDE_LISTS) {
			try {
				...
				Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to load hide player registry: {}", error));
			}
		}
	}
}