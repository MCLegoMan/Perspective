...
public class TexturedEntityDataLoader extends JsonDataLoader implements IdentifiableResourceReloadListener {
	...
	private void add(String ENTITY, String NAME, Boolean ENABLED) {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to add textured entity to registry: {}", error));
		}
	}
	private void reset() {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to reset textured entity registry: {}", error));
		}
	}
	private void add$default() {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to add default textured entities to registry: {}", error));
		}
	}
	@Override
	public void apply(Map<Identifier, JsonElement> prepared, ResourceManager manager, Profiler profiler) {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to apply textured entity registry: {}", error));
		}
	}
	@Override
	...
	private void layout$perspective(Identifier identifier, JsonElement jsonElement) {
		try {
			...
			Data.version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to load textured entity to registry: {}", error));
		}
	}
}