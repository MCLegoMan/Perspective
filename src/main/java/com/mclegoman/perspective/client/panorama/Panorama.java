	...
	private static void takePanorama(int resolution) {
		if (ClientData.minecraft.player != null) {
			try {
				if (shouldTakePanorama()) {
					...
					if (new File(assetsDirLoc).mkdirs()) {
						...
						for (int l = 0; l < 6; ++l) {
							...
							ClientData.minecraft.gameRenderer.render(1.0F, 0L, true);
							try {
								if ((boolean) ConfigHelper.getConfig("super_secret_settings_enabled")) {
									...
									panoramaPerspectivePostProcessor.render(ClientData.minecraft.getTickDelta());
								}
							} catch (Exception error) {
								Data.version.sendToLog(Helper.LogType.ERROR, "Failed to add Super Secret Settings to Panorama.");
							}
					...
					if (!getIncompatibleMods().isEmpty() || ClientData.minecraft.options.getGraphicsMode().getValue().equals(GraphicsMode.FABULOUS)) {
						if ((boolean) ConfigHelper.getConfig("debug")) {
							Data.version.sendToLog(Helper.LogType.WARN, "Debug is enabled. The panorama screenshot might not have rendered as expected.");
					...
					if (!getIncompatibleMods().isEmpty()) {
						Data.version.sendToLog(Helper.LogType.WARN, Translation.getString("An error occurred whilst trying to take a panorama: Incompatible Mod(s): {}", getIncompatibleMods().toString().replace("[", "").replace("]", "")));
					...
					if (ClientData.minecraft.options.getGraphicsMode().getValue().equals(GraphicsMode.FABULOUS)) {
						Data.version.sendToLog(Helper.LogType.WARN, Translation.getString("An error occurred whilst trying to take a panorama: Take Panorama Screenshot is not compatible with Fabulous graphics.", Data.version.getLoggerPrefix()));
				...
				Data.version.sendToLog(Helper.LogType.WARN, Translation.getString("An error occurred whilst trying to take a panorama: {}", error));
				//ClientData.minecraft.getToastManager().add(new SystemToast(Translation.getTranslation(Data.version.getID(), "toasts.title", new Object[]{Translation.getTranslation(Data.version.getID(), "name"), Translation.getTranslation(Data.version.getID(), "toasts.take_panorama_screenshot.failure.title")}), Text.of(String.valueOf(error)), 320));
			}
		}
	}
}