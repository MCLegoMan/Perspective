...
public class ShaderDataLoader extends JsonDataLoader implements IdentifiableResourceReloadListener {
	...
	private void add(String NAMESPACE, String SHADER_NAME, boolean DISABLE_SCREEN_MODE, boolean ENABLED) {
		try {
			...
			Identifier ID = Identifier.of(NAMESPACE.toLowerCase(), ("shaders/post/" + SHADER_NAME + ".json"));
			...
			Data.version.sendToLog(LogType.ERROR, Translation.getString("Failed to add shader to registry: {}", error));
		}
	}
	private void reset() {
		try {
			...
			Data.version.sendToLog(LogType.ERROR, Translation.getString("Failed to reset shader registry: {}", error));
		}
	}
	private void add$default() {
		try {
			...
			Data.version.sendToLog(LogType.ERROR, Translation.getString("Failed to add default shaders to registry: {}", error));
		}
	}
	@Override
	public void apply(Map<Identifier, JsonElement> prepared, ResourceManager manager, Profiler profiler) {
		try {
			...
			Data.version.sendToLog(LogType.ERROR, Translation.getString("Failed to apply shader registry: {}", error));
		}
	}
	@Override
	...
	private void layout$souper_secret_settings(ResourceManager manager) {
		List<Resource> SHADER_LISTS = manager.getAllResources(Identifier.of("souper_secret_settings", "shaders.json"));
		for (Resource resource : SHADER_LISTS) {
			try {
				...
				Data.version.sendToLog(LogType.ERROR, Translation.getString("Failed to load soup shader: {}", error));
			}
		}
	}
}