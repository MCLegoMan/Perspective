...
public class Data {
	public static final Version version = new Version("Perspective", "perspective", 1, 2, 3, Helper.ReleaseType.RELEASE, 1);
	...
	public static boolean isModInstalledVersionOrHigher(String modId, String REQUIRED_VERSION, boolean SUBSTRING) {
		try {
			...
			version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to check mod version for " + modId + ": {}", error));
		}
		return false;
	}
	public static boolean checkModVersion(String CURRENT_VERSION, String REQUIRED_VERSION, boolean SUBSTRING) {
		try {
			...
			version.sendToLog(Helper.LogType.ERROR, Translation.getString("Failed to check mod version: {}", error));
		}
		return false;
	}
